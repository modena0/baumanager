...
export function TeamplanungTab({ data, currentUser, rolle }: any) {
  ...
  async function loadAll() {
    setLoading(true);
    // Lade Teams für gewähltes Datum (einmalige + dauerhafte die in Zeitraum fallen)
  ...
  // Team-Chemie aktualisieren wenn Team gespeichert wird
  ...
  function kiOptimierung() {
    ...
    const priorisierteChemie = chemie.filter(c => c.priorisiert).map(c => {
      ...
      return a && b ? `${a.name} + ${b.name} (${c.zusammen_count}x zusammen, priorisiert)` : null;
    }).filter(Boolean);

    const haeufigChemie = chemie.filter(c => !c.priorisiert && c.zusammen_count >= 3).map(c => {
      const a = data.mitarbeiter.find((m: any) => m.id === c.mitarbeiter_a);
      const b = data.mitarbeiter.find((m: any) => m.id === c.mitarbeiter_b);
      return a && b ? `${a.name} + ${b.name} (${c.zusammen_count}x zusammen)` : null;
    }).filter(Boolean);

    const prompt = `Du bist ein Bau-Logistik-Experte. Erstelle einen optimalen Teamplan für den ${datum}.

MITARBEITER:
${JSON.stringify(data.mitarbeiter.filter((m: any) => m.status !== "krank" && m.status !== "Urlaub" && m.status !== "gekuendigt").map((m: any) => ({ id: m.id, name: m.name, rolle: m.rolle, qualifikationen: m.qualifikationen, fuehrerschein: m.fuehrerschein, nichtMit: m.nichtMit })))}

BAUSTELLEN (aktiv):
${JSON.stringify(data.baustellen.filter((b: any) => b.status !== "abgeschlossen").map((b: any) => ({ id: b.id, name: b.name, anforderungen: b.anforderungen })))}

PRIORISIERTE TEAM-PAARE (diese unbedingt zusammenplanen):
${priorisierteChemie.length > 0 ? priorisierteChemie.join("\n") : "Keine"}

HÄUFIGE TEAM-PAARE (wenn möglich zusammenplanen):
${haeufigChemie.length > 0 ? haeufigChemie.join("\n") : "Keine"}

REGELN:
- nichtMit Mitarbeiter NIEMALS im gleichen Team
- Priorisierte Paare immer zusammenplanen
- Teams von 2-4 Personen
- LKW-Fahrer können rotierend zwischen Baustellen eingesetzt werden
- Qualifikationen mit Baustellen-Anforderungen abgleichen
- Antworte auf Deutsch mit konkreten Team-Vorschlägen (Team-Name, Mitglieder, Baustelle, ob jemand rotierend ist)`;
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        ...
        {kannPlanen && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button style={{ ...C.btnS }} onClick={() => setShowChemie(true)}>⚗ Team-Chemie</button>
            <button style={{ ...C.btnS }} onClick={kiOptimierung}>✦ KI-Vorschlag</button>
            <button style={{ ...C.btnP }} onClick={() => {
              setEditTeam({ datum, datum_bis: "", ist_dauerhaft: false, spezialisierung: "", team_name: "", farbe: TEAM_FARBEN[teams.length % TEAM_FARBEN.length], mitarbeiter: [], rotierende: [], baustellen: [], notizen: "", erstellt_von: "" });
              setShowForm(true);
            }}>+ Neues Team</button>
          </div>
        )}
      ...
      {(aiLoad || aiRes) && (
        <div style={{ ...C.card, marginBottom: 16, background: "#f8fffe", border: "1px solid " + ACCENT + "44" }}>
          ...
          <div style={{ fontSize: 13, lineHeight: 1.7, color: aiLoad ? "#bbb" : "#444", whiteSpace: "pre-wrap" }}>
            {aiLoad ? "KI erstellt optimalen Teamplan..." : aiRes}
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 14 }}>
          {teams.map(team => (
            <div key={team.id} style={{ ...C.card, borderTop: "4px solid " + team.farbe, padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                <div>
                  ...
                  {team.ist_dauerhaft && team.datum_bis && (
                    <div style={{ fontSize: 10, color: "#bbb", marginTop: 2 }}>{team.datum} – {team.datum_bis}</div>
                  )}
                </div>
                {kannPlanen && (
                  <div style={{ display: "flex", gap: 5 }}>
                    <button onClick={() => { setEditTeam(team); setShowForm(true); }} style={{ padding: "4px 8px", borderRadius: 7, border: "1px solid #eee", background: "#fff", cursor: "pointer", fontSize: 11, color: "#555" }}>✎</button>
                    <button onClick={() => deleteTeam(team.id!)} style={{ padding: "4px 8px", borderRadius: 7, border: "none", background: "#E24B4A18", cursor: "pointer", fontSize: 11, color: "#E24B4A" }}>✕</button>
                  </div>
                )}
              </div>
              ...
              <div style={{ marginBottom: team.notizen ? 10 : 0 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                  {team.baustellen.map(id => {
                    const b = data.baustellen.find((x: any) => x.id === id);
                    if (!b) return null;
...
function TeamFormular({ team, data, chemie, onSave, onClose }: any) {
  ...
  ).filter((c: any) => c.priorisiert || c.zusammen_count >= 3);
      ...
      <div style={{ ...C.card, width: isMobile ? "100%" : "min(620px,95vw)", maxHeight: isMobile ? "92vh" : "90vh", overflowY: "auto", borderRadius: isMobile ? "20px 20px 0 0" : 16, padding: isMobile ? "20px 16px 32px" : 24, margin: 0 }}>
        ...
        <div style={{ marginBottom: 14, padding: "12px 14px", background: "#f8f8ff", borderRadius: 10, border: "1px solid #e8eaed" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: f.ist_dauerhaft ? 10 : 0 }}>
            ...
            <label htmlFor="dauerhaft" style={{ fontSize: 13, fontWeight: 600, color: "#333", cursor: "pointer" }}>
              Stamm-Team (dauerhaft für Zeitraum)
            </label>
          </div>
          {f.ist_dauerhaft && (
        ...
        {chemieHinweise.length > 0 && (
          <div style={{ marginBottom: 14, padding: "10px 12px", background: "#fff8e1", borderRadius: 10, border: "1px solid #BA751733" }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: "#BA7517", marginBottom: 6 }}>⚗ Team-Chemie Hinweise</div>
            {chemieHinweise.map((c: any) => (
              <div key={c.id} style={{ fontSize: 11, color: "#666", marginBottom: 2 }}>
                {c.priorisiert ? "⭐" : "↻"} {getMaName(c.mitarbeiter_a)} + {getMaName(c.mitarbeiter_b)} – {c.zusammen_count}x zusammen gearbeitet
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: isMobile ? "flex-end" : "center", justifyContent: "center", zIndex: 9999 }}
      ...
      <div style={{ ...C.card, width: isMobile ? "100%" : "min(500px,95vw)", maxHeight: isMobile ? "85vh" : "80vh", overflowY: "auto", borderRadius: isMobile ? "20px 20px 0 0" : 16, padding: isMobile ? "20px 16px 32px" : 24, margin: 0 }}>
        ...
        <div style={{ fontSize: 12, color: "#aaa", marginBottom: 16 }}>Wird automatisch gespeichert wenn Teams eingeplant werden. Priorisierte Paare werden bei KI-Vorschlägen bevorzugt.</div>
          ...
          <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 10, border: "1px solid " + (c.priorisiert ? "#BA751733" : "#eee"), background: c.priorisiert ? "#fff8e1" : "#fff", marginBottom: 6 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 500, color: "#222" }}>
                {getMaName(c.mitarbeiter_a)} + {getMaName(c.mitarbeiter_b)}
              </div>
