...
const fixBS = (b: any) => ({
  ...b,
  anforderungen: Array.isArray(b.anforderungen) ? b.anforderungen : [],
...
export function BaustellenTab({ data, setData, openEdit, deleteItem, selectedBS, setSelectedBS, rolle, currentUser, isMobile }: any) {
  ...
  const assignMA = async (bsId: number, maId: number) => {
    ...
    const neuListe = was ? liste.filter((i: number) => i !== maId) : [...liste, maId];
  ...
  const assignFZ = async (bsId: number, fzId: number) => {
    ...
    const neuListe = was ? liste.filter((i: number) => i !== fzId) : [...liste, fzId];
  ...
  const assignEQ = async (bsId: number, lgId: number) => {
    ...
    const neuListe = was ? liste.filter((i: number) => i !== lgId) : [...liste, lgId];
    const lager = data.lager.find((l: any) => l.id === lgId);
    const neuVerfuegbar = was ? Math.min(lager.anzahl, (lager.verfuegbar || 0) + 1) : Math.max(0, (lager.verfuegbar || 0) - 1);
  ...
  async function speichernNeu(formData: any) {
    ...
    if (neu) {
      setData((d: any) => ({ ...d, baustellen: [...d.baustellen, fixBS(neu[0])] }));
      // Auto-Geocoding
      if (formData.ort) {
        try {
  ...
  const anzeigeBS = isBauleitung
    ? data.baustellen.filter((b: any) => toNumArr(b.mitarbeiter || []).includes(currentUser.id))
    : selectedBS
