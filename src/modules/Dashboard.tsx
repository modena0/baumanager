              ...
              <div key={b.id} style={{ background: col + "44", borderRadius: 12, padding: "12px 16px", marginBottom: 10, border: "1.5px solid " + col }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "#888", marginTop: 1 }}>{b.ort} - {b.kategorie}</div>
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 12 }}>
                    <div style={{ fontSize: 20, fontWeight: 700, color: ACCENT }}>{prog}%</div>
                    <div style={{ fontSize: 10, color: dl <= 14 ? "#E24B4A" : "#aaa" }}>{dl}d</div>
                  </div>
                </div>
                <div style={{ height: 4, background: "rgba(0,0,0,0.08)", borderRadius: 2, marginBottom: 6 }}>
                  <div style={{ height: "100%", width: prog + "%", background: ACCENT, borderRadius: 2 }} />
                </div>
                {auf.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                    {auf.slice(0, 4).map((a: any) => (
                      <span key={a.id} style={{ fontSize: 10, padding: "1px 7px", borderRadius: 10, background: a.erledigt ? ACCENT + "22" : "#f0f0f0", color: a.erledigt ? ACCENT : "#999", textDecoration: a.erledigt ? "line-through" : "none" }}>{a.titel}</span>
                    ))}
                    {auf.length > 4 && <span style={{ fontSize: 10, color: "#bbb" }}>+{auf.length - 4}</span>}
                    <span style={{ fontSize: 10, color: "#bbb", marginLeft: "auto" }}>{done}/{auf.length} erledigt</span>
                  </div>
                )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={C.card}>
            ...
            {heuteTermine.map((t: any) => (
              <div key={t.id} style={{ display: "flex", gap: 12, padding: "8px 0", borderBottom: "1px solid #f5f5f5", alignItems: "center" }}>
                <div style={{ width: 4, minHeight: 32, borderRadius: 2, background: t.art === "Pflichttermin" ? "#E24B4A" : ACCENT, flexShrink: 0 }} />
                <div>
        ...
        <div style={C.card}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
            <input
              style={{ ...C.inp, marginBottom: 0, flex: 1, fontSize: 12 }}
              value={frage}
          ...
          <div style={{ background: "#f8fffe", borderRadius: 10, padding: 12, minHeight: 60, fontSize: 12, lineHeight: 1.7, color: aiLoad ? "#bbb" : "#444", whiteSpace: "pre-wrap", border: "1px solid #e8f5f3" }}>
