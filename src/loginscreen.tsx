      ...
      <div style={{ ...C.card, width: "min(400px,95vw)", textAlign: "center" }}>
        ...
        <div style={{ marginBottom: 12, textAlign: "left" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 200, overflowY: "auto" }}>
            {loginUser.map((m: any) => (
              <button key={m.id} onClick={() => { setSelected(m); setPin(""); setFehler(""); }}
                style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", borderRadius: 10, border: "1.5px solid " + (selected?.id === m.id ? ACCENT : "#e8eaed"), background: selected?.id === m.id ? "#e8f5f3" : "#fff", cursor: "pointer", textAlign: "left" }}
              >
                <div>
        ...
        <button onClick={handleLogin} disabled={!selected || pin.length < 4}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
            {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((k, i) => (
              <button key={i}
                ...
                style={{ padding: "14px", borderRadius: 10, border: k === "" ? "none" : "1.5px solid #e8eaed", background: k === "" ? "transparent" : "#fff", cursor: k === "" ? "default" : "pointer", fontSize: 18, fontWeight: 500, color: "#333" } as any}
              >{k}</button>
            ))}
          </div>
        )}
