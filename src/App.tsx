...
export default function App() {
  ...
  const openEdit = (type: string, item: any) => {
    ...
    if (type === "mitarbeiter") {
      ...
      f.gutMit   = toNumArr(item.gutMit).map((id: number) => { const m = data.mitarbeiter.find((x: any) => x.id === id); return m ? m.name : ""; }).filter(Boolean).join(", ");
      f.nichtMit = toNumArr(item.nichtMit).map((id: number) => { const m = data.mitarbeiter.find((x: any) => x.id === id); return m ? m.name : ""; }).filter(Boolean).join(", ");
    }
  ...
  const saveItem = async (f: any) => {
    ...
    if (type === "baustellen") {
      ...
      p.fahrzeuge     = Array.isArray(p.fahrzeuge)   ? p.fahrzeuge.map(Number)   : [];
