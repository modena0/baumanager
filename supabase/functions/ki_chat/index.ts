  ...
  try {
    ...
    if (file_ids && file_ids.length > 0) {
      for (const file_id of file_ids) {
        content.push({ type: "document", source: { type: "file", file_id } })
      }
    }
    if (dokumente && dokumente.length > 0) {
      for (const dok of dokumente) {
        if (dok.type === "image_base64") {
          content.push({ type: "image", source: { type: "base64", media_type: dok.media_type || "image/jpeg", data: dok.data } })
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: max_tokens || 1000, system: system || "Du bist ein Assistent.", messages: [{ role: "user", content }] }),
    })
